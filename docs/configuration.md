...
| `RAG_LIVE_DATA_BUDGET_BYTES` | `32768` | Ceiling on the **LIVE DATA** block — output from MCP tools — spliced into a synthesis prompt. A live tool can return far more than a model can read: measured 2026-09-04, one Jira search returned 68,779 bytes and one Confluence search 103,857, against a model loaded with a 16,384-token window. Over-budget output is truncated with an explicit marker and a `warn` log (`live tool output exceeds the synthesis budget`), so the loss is visible instead of the provider dropping it silently and the answer reading as "nothing found". Raise it for large-window models; lower it if answers degrade when a tool returns a lot. |
